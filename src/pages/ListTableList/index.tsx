...
import { dracula, codepenEmbed, atomOneDark } from 'react-syntax-highlighter/dist/esm/styles/hljs';
        ...
        rules: [
          {
            required: true,
            message: (
              <FormattedMessage id="pages.searchTable.ruleName" defaultMessage="题目为必填项" />
            ),
          },
        ],
      },
      render: (dom, entity) => {
        return <a onClick={() => setRow(entity)}>{dom.replace('## ', '')}</a>;
      },
    },
    {
      title: <FormattedMessage id="pages.searchTable.titleDesc" defaultMessage="答案" />,
      dataIndex: 'content',
      valueType: 'textarea',
      render: (dom, entity) => {
      console.log(dom)
      return <ReactMarkdown renderers={renderers}>{dom.props.text}</ReactMarkdown>
      }
    },
